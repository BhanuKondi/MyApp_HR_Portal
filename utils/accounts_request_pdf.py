...
def _kv_table(rows):
    table = Table(rows, colWidths=[1.9 * inch, 4.9 * inch], hAlign="LEFT")
    table.setStyle(
        TableStyle(
            [
...
def _effective_payment_date(accounts_request):
    if getattr(accounts_request, "payment_date", None):
        return accounts_request.payment_date
    if getattr(accounts_request, "expense_recorded_at", None):
        return accounts_request.expense_recorded_at.date()
    return None
...
def _append_attachment_previews(story, accounts_request, styles):
    story.append(Paragraph("Attachments", styles["SectionHeading"]))
    if not accounts_request.attachments:
        story.append(Paragraph("No attachments uploaded.", styles["BodyText"]))
        return

    for attachment in accounts_request.attachments:
        stage_label = attachment.attachment_stage.replace("_", " ").title()
        story.append(Paragraph(f"{attachment.file_name} ({stage_label})", styles["BodyText"]))
        extension = os.path.splitext(attachment.file_path or "")[1].lower()
        absolute_path = _attachment_absolute_path(attachment.file_path)
...
def render_accounts_request_pdf(accounts_request, viewer_role: str):
    ...
    company = getattr(accounts_request, "company", None)

    story.append(Paragraph(company.legal_name if company else "ATIKES", styles["Title"]))
    story.append(Paragraph("Accounts Request Summary", styles["Heading2"]))
    story.append(
        Paragraph(
            f"{accounts_request.request_no} | Generated for {viewer_role.replace('_', ' ').title()}",
            styles["MutedText"],
        )
    )
    ...
    summary_rows = [
        ...
        ["Approver", _safe_user_label(accounts_request.approver)],
        ["Type", accounts_request.request_type.name if accounts_request.request_type else "-"],
        ["Title", accounts_request.title],
        ["Payment Mode", accounts_request.payment_mode.replace("_", " ").title() if accounts_request.payment_mode else "-"],
        ["Payment Date", _format_date(_effective_payment_date(accounts_request))],
        ["Vendor / Payee", accounts_request.vendor_name or "-"],
        ["Requested Amount", _format_amount(accounts_request.requested_amount)],
        ["Approved Amount", _format_amount(accounts_request.approved_amount)],
        ["Actual Amount", _format_amount(accounts_request.actual_amount)],
        ["Payment Reference", accounts_request.payment_reference or "-"],
        ["Status", accounts_request.status.replace("_", " ").title()],
        ["Description", accounts_request.description or "-"],
    ]
    ...
    story.append(Paragraph("Approval And Closure Notes", styles["SectionHeading"]))
    story.append(
        _kv_table(
            [
                ["Approval Comments", accounts_request.approval_comments or "-"],
                ["Execution Comments", accounts_request.execution_comments or "-"],
                ["Closure Comments", accounts_request.closure_comments or "-"],
            ]
        )
    )
    story.append(Spacer(1, 14))

    _append_attachment_previews(story, accounts_request, styles)
    attachment_pdf_paths = _collect_attachment_pdf_paths(accounts_request.attachments)
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename={_safe_company_code(company).lower()}-{accounts_request.request_no.lower()}-summary.pdf"
    )
    return response
